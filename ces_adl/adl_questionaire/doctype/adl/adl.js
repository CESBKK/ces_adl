...
setup_queationaire = (frm) => {
    const questions = [
        ...
        },
        {
            ...
            options: [
                ...
                'ต้องการความช่วยเหลืออย่างมาก เช่น ต้องใช้คนแข็งแรงหรือมีทักษะ 1 คน หรือใช้คนทั่วไป 2 คน พยุงหรือดันขึ้นมาจึงจะนั่งอยู่ได้',
    ...
    questions.forEach((question) => {
        frm.fields_dict[question.fieldname] = frappe.ui.form.make_control({
        ...
        frm.question_list.push(frm.fields_dict[question.fieldname]);
    });
...
sync_quetionaire_json = (frm) => {
    ...
    frm.question_list.forEach((item) => {
        ...
        if (jsonData[item.df.fieldname] !== '' && jsonData[item.df.fieldname] >= 0) {
            ...
            frm.get_field(item.df.fieldname).$wrapper.find(`#${radioId}`).prop('checked', true);
        } else {
            frm.get_field(item.df.fieldname).$wrapper.find(`input[name="${item.df.fieldname}Group"]`).prop('checked', false);
        }
    });
};
...
make_radio_button = (frm) => {
    frm.question_list.forEach((item) => {
        const field = frm.get_field(item.df.fieldname);
        ...
        if (field.$wrapper.find(`#${container_id}`).length) {
          return;
        }

        const container = $(`<div id="${container_id}" class="radioSelectContainer" />`);
        field.$wrapper.find('.control-label').addClass('ces-adl-question');
        field.$wrapper.append(container);
        container.append(select);
        select.hide();
        field.$wrapper.find('.select-icon').hide();

        select.find('option').each(function (optionIndex, optionElement) {
            const optionValue = $(this).val();
            if (optionValue !== '') {
                const radio_container = $(`<div class="mb-2"/>`);
                radio_container.appendTo(container);

                const element_id = `${item.df.fieldname}-${(parseInt(optionIndex) - 1)}`;
                const $radio = $(`<input type="radio" name="${radioGroup}" id="${element_id}" value="${optionValue}" />`);
                const $label = $(`<label for="${element_id}" class="ml-2">${$(this).text()}</label>`);

                $radio.on('change', function () {
                    const isChecked = this.checked;
                    select.val(isChecked ? $(this).val() : '');
                    select.trigger('change');
                    frm.refresh_field(item.df.fieldname);
                });
